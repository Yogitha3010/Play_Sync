...
class TurfOwnerBookingsScreen extends StatefulWidget {
  @override
  _TurfOwnerBookingsScreenState createState() => _TurfOwnerBookingsScreenState();
}
...
class _TurfOwnerBookingsScreenState extends State<TurfOwnerBookingsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // 2. Get bookings for each turf
      final Map<String, List<BookingModel>> bookingsMap = {};
      for (var turf in turfs) {
        final turfBookings = await _firestoreService.getTurfBookings(turf.turfId);
        if (turfBookings.isNotEmpty) {
          bookingsMap[turf.turfId] = turfBookings;
        }
      }
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text('No bookings found for your turfs.', style: TextStyle(color: Colors.grey, fontSize: 16)),
                    ],
                  ),
                )
              : RefreshIndicator(
                      ...
                      return Column(
                        ...
                        children: [
                          Padding(
                            ...
                            child: Row(
                              children: [
                                Icon(Icons.stadium, color: AppTheme.theme.primaryColor),
                                SizedBox(width: 8),
                                Text(
                                  turf.name,
                          ...
                          ...turfBookings.map((booking) => _buildBookingCard(booking, turf)).toList(),
                        ],
                      );
                    },
                  ),
